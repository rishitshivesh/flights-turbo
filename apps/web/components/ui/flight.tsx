...
import {MapMarker, MapPopup, MarkerContent, MarkerLabel, useMap,} from "@/components/ui/map";
import {cn} from "@/lib/utils";

import {type AirportInfo, type AirportRef, airports} from "./flight-airports";
...
type FlightAirportProps = {
    ...
    className?: string;
    airportInfo?: AirportInfo; // Optional pre-resolved airport info to avoid lookup
    /**
...
function FlightAirport({
    ...
    // Resolve coordinates
    const [airportInfo, setAirportInfo] = useState<AirportInfo | undefined>(undefined);
    const [airportInfoOverride, setAirportInfoOverride] = useState<boolean>(false);

    useEffect(() => {
        if (airportInfoProp && airportInfoProp.code) {
            setAirportInfo(airportInfoProp);
            setAirportInfoOverride(true);
        } else if (code) {
            setAirportInfo(getAirportInfo(code))
        } else setAirportInfo(undefined);

    }, [code, lngProp, latProp, airportInfoProp]);


    const lng = airportInfoOverride ? airportInfo?.longitude : lngProp ?? airportInfo?.longitude;
    const lat = airportInfoOverride ? airportInfo?.latitude : latProp ?? airportInfo?.latitude;
    const displayName = airportInfoOverride ? (airportInfo ? airportInfo.code : undefined) : nameProp ?? (airportInfo ? airportInfo.code : undefined);


    console.log({displayName, showLabel})
    ...
    return (
        <MapMarker longitude={lng as number} latitude={lat as number} onClick={handleClick}>
...
type FlightRouteProps = {
    ...
    airportInfo?: { from: AirportInfo; to: AirportInfo };
};
...
function FlightRoute({
    ...
    useEffect(() => {
        ...
        map.setPaintProperty(layerId, "line-dasharray", (resolvedDash as any) ?? null);

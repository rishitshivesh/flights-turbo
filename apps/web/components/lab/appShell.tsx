                ...
                return (
                  ...
                  >
                    ...
                    {!collapsed && !item.ready && (
                      <span className={cn('ml-auto rounded px-1.5 py-0.5 text-[9px] font-medium', active ? 'bg-background/15' : 'bg-muted-foreground/10')}>
                        TODO
                      </span>
                    )}
                  </Link>
    ...
    <div className="min-h-dvh bg-background text-foreground">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <button className="absolute inset-0 bg-black/45 backdrop-blur-sm" onClick={() => setMobileOpen(false)} aria-label="Close menu" />
          <div className="relative w-[280px]">{sidebar}</div>
          <button className="absolute right-4 top-4 rounded-full bg-background p-2 shadow-lg" onClick={() => setMobileOpen(false)}>
            <X className="size-4" />
          </button>
        </div>
      )}
      ...
      <div className={cn('transition-[padding] duration-200', collapsed ? 'lg:pl-[72px]' : 'lg:pl-[256px]')}>
        <header className="sticky top-0 z-30 flex h-14 items-center border-b border-border/50 bg-background/80 px-4 backdrop-blur-xl lg:px-6">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span>SQL Lab</span>
            <span>/</span>
            <span className="max-w-[260px] truncate text-foreground">{pathname === '/' ? 'overview' : pathname.replace(/^\//, '')}</span>
          </div>
        </header>

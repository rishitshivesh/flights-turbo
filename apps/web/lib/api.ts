...
import { FilterOption } from "@/components/managed";
...
export function getAirports(
  ...
  return apiFetch(
    `/api/airports${searchParams.size ? `?${searchParams.toString()}` : ""}`,
  );
}
...
export function getRoutes(
  ...
  if (params.departureAirport)
    search.set("departureAirport", params.departureAirport);
  if (params.arrivalAirport)
    search.set("arrivalAirport", params.arrivalAirport);
...
export function getStaticData(
  ...
  if (params.search) searchParams.set("search", params.search);
  return apiFetch(
    `/api/airports/static/${key}${searchParams.size ? `?${searchParams.toString()}` : ""}`,
  );
}
